"""
Scraper para Easy Colombia (Cencosud). VTEX API.
"""
...
API_BASE = "https://www.easy.com.co/api/catalog_system/pub/products/search"

PALABRAS_EXCLUIR = {
    "organizador", "abrazadera", "repuesto", "manguera", "soporte",
    "accesorio", "cubierta", "funda", "cesto", "canasta",
    "iman", "iman", "adorno", "protector de voltaje",
}

CATEGORIAS = {
    "neveras":              ["nevera","nevera samsung","nevera lg","nevera mabe","nevera haceb","nevecon"],
...
def scrape_termino(termino: str, categoria: str) -> list[dict]:
    productos, desde, por_pagina = [], 0, 50
    while True:
        try:
        ...
        for item in items:
            ...
            if not any(p in nombre_lower for p in termino.lower().split()): continue
            if any(ex in nombre_lower for ex in PALABRAS_EXCLUIR): continue
            try:
                ...
                if not precio or not oferta.get("IsAvailable", False): continue
                imagenes = item.get("items", [{}])[0].get("images", [])
                prod = {
                    "nombre": nombre, "precio": float(precio),
                    "url": f"https://www.easy.com.co/{item.get('linkText','')}/p",
                    "imagen": imagenes[0].get("imageUrl", "") if imagenes else "",
                    "tienda": "Easy", "categoria": categoria,
                }
